...
int main(int argc, char* argv[]) {
  ...
  */
  ...
  for(int i = 0; i < row; i++){
    ...
    for(int j = 0; j < num_of_C; j++) var_cT_beta_hat[i][j] =  new double[dimensions[0] * dimensions[1] * dimensions[2]]();
  }
  ...
  for(int i = 0; i < row; i++){
    ...
    for(int j = 0; j < col; j++) _beta_hat[i][j] = new double[dimensions[0] * dimensions[1] * dimensions[2]]();
  }
  ...
  for(int i = 0; i < row; i++){
    ...
    for(int j = 0; j < num_of_C; j++) Z_score[i][j] = new double[dimensions[0] * dimensions[1] * dimensions[2]]();
  }

  double*** SPRT = new double**[row];
  for(int i = 0; i < row; i++){
    SPRT[i] = new double*[num_of_C];
    for(int j = 0; j < num_of_C; j++) SPRT[i][j] = new double[dimensions[0] * dimensions[1] * dimensions[2]]();
  }
    ...
    cilk_for (int x = 0; x < dimensions[0]; x++) {
      for (int y = 0; y < dimensions[1]; y++) {
        for (int z = 0; z < dimensions[2]; z++) {
          if(is_within_ROI[x * dimensions[1] * dimensions[2] + y * dimensions[2] + z]){ // only prepare data and run SPRT when voxel is within ROI region
            ...
            for(int i=0; i<num_of_C; i++) var_cT_beta_hat[scan_number-1][i][pos] = compute_var_cTbeta_hat_sparse(C[i], X, XTX_inverse_XT, X_XTX_inverse, 
  ...
  if(toupper(output_specific_voxles) == 'Y'){
    ...
    if(voxels_list.size() % 3 == 0){
      for(int i=0; i<num_of_C; i++){
        ...
        myfile1 << "This is specific voxel information for contrast" << to_string(i+1) << endl << endl;
        ...
        myfile1 << "SPRT 95% confidence intervals" << endl << "In Result section, -1 means less than left 2.5% CI, 1 means greater than right 2.5% CI, 0 means in between (Undecided)" << endl;
        myfile1 << "Label,X,Y,Z,";
        for(int scan = K_blk+1; scan <= row; scan++) myfile1 << "Scan" << scan << ",";
        myfile1 << endl;
        for(int in = 0; in < voxels_list.size()/3; in++){
          ...
          for(int scan = K_blk+1; scan <= row; scan++){
            if(_beta_hat[scan-1][i][pos]) myfile1 << "-1,";
            else if(_beta_hat[scan-1][i][pos]) myfile1 << "1,";
            else myfile1 << "0,";
          }
          myfile1 << endl;
        }
        ...
        myfile1 << "Statistics from scan 119 is used for this estimation" << endl << endl;
  ...
  for(int i=0; i<num_of_C; i++){
    ...
    myfile1 << ",";
    for(int scan_number = K_blk; scan_number < row; scan_number++) myfile1 << "Scan " << to_string(scan_number+1) << ",";
    myfile1 << endl << "Cross, Upper, Bound, Voxels, :" << endl;
    for (int x = 0; x < dimensions[0]; x++) {
      for (int y = 0; y < dimensions[1]; y++) {
        for (int z = 0; z < dimensions[2]; z++) {
    ...
    myfile1 << endl << "Cross, Lower, Bound, Voxels, :" << endl;
    for (int x = 0; x < dimensions[0]; x++) {
      for (int y = 0; y < dimensions[1]; y++) {
        for (int z = 0; z < dimensions[2]; z++) {
    ...
    myfile1 << endl << "No, Bound, Crossed, Voxels, :" << endl;
    for (int x = 0; x < dimensions[0]; x++) {
      for (int y = 0; y < dimensions[1]; y++) {
        for (int z = 0; z < dimensions[2]; z++) {
  ...
  cout << ".  .  ." << endl << ".  .  ." << endl << ".  .  ." << endl << "SPRT analysis finished." << endl;
