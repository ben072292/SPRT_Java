...
public class SparkDriver implements Serializable {
	...
	public static void main(String[] args) {
		...
		// configure spark
		// SparkConf sparkConf = new SparkConf().setAppName("Distributed
		// SPRT").setMaster("local[2]")
		...
		for (scanNumber = config.K + 1; scanNumber <= config.ROW; scanNumber++) {
			...
			// System.out.println(new Date() + ": Round " + scanNumber + ": Retriving data
			// from activationMap RDD using take()");
