...
public class Driver {
	public static void main(String[] args) {
		...
		// Prepare
		System.out
				.println("Successfully reading in first " + config.K + " scans, Now start preparing SPRT estimation.");
		...
		for (int i = 0; i < contrasts.numContrasts; i++) {
			...
			// theta1Dataset[i] = new Dataset(config.ROW, config.getX(), config.getY(),
			// config.getZ());
			// ZScoreDataset[i] = new Dataset(config.ROW, config.getX(), config.getY(),
			// config.getZ());
			// varianceDataset[i] = new Dataset(config.ROW, config.getX(), config.getY(),
			// config.getZ());
			// SPRTDataset[i] = new Dataset(config.ROW, config.getX(), config.getY(),
			// config.getZ());
			// betaDataset[i] = new Dataset(config.ROW, config.getX(), config.getY(),
			// config.getZ());
		}
		...
		for (scanNumber = config.K + 1; scanNumber <= config.ROW; scanNumber++) {
					...
					for (int z = 0; z < dataset.getZ(); z++) {
						if (ROI[x * dataset.getY() * dataset.getZ() + y * dataset.getZ() + z]) {
							...
							for (int i = 0; i < C.getRow(); i++) {
								...
								double SPRTActivationStatus = Numerical.computeActivationStatus(SPRT,
										config.SPRTUpperBound, config.SPRTLowerBound);
								if (SPRTActivationStatus == 1)
									counter++;
			...
			for (int i = 0; i < C.getRow(); i++) {
				...
				for (int x = 0; x < dataset.getX(); x++) {
					for (int y = 0; y < dataset.getY(); y++) {
						for (int z = 0; z < dataset.getZ(); z++) {
							if (activationDataset[i].getSignal(scanNumber, x, y, z) == 1)
								activatedCounter++;
						}
					}
				}
