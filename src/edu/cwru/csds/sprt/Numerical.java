...
public class Numerical {
	...
	public static double computeVarianceUsingMKLSparseRoutine1(Matrix c, Matrix X, Matrix D) {
		try {
			return c.multiply(X.transposeMultiply(X).inverse()).multiplyTranspose(X).multiply(D.sparseMultiplyDense(X.multiply(X.transposeMultiply(X).inverse()))).multiplyTranspose(c).get();
		} catch (MatrixComputationErrorException e) {
			e.printStackTrace();
		}
		System.err.println("Error: You should never see this line, go debug");
	 ...
	 * ~200 times faster than without using sparse BLAS routines.
	 */
	public static double computeVarianceUsingMKLSparseRoutine2(Matrix c, Matrix XTXInverseXT, Matrix XXTXInverse, Matrix D) {
	...
	/*
	 * Output: SPRT result
	 * Functionality: this function is to handle the weight lifting part and computes
