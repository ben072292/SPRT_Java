...
public class Dataset implements Serializable {
	...
	public void addOneScan(Brain volume) {
		try {
			...
			if (this.x != volume.getX() || this.y != volume.getY() || this.z != volume.getZ())
		...
		for (int x = 0; x < this.x; x++) {
			for (int y = 0; y < this.y; y++) {
				for (int z = 0; z < this.z; z++) {
					int pos = getLocation(x, y, z);
					if (ROI[pos]) {
						ret.add(new DistributedDataset(getSingleBoldResponse(scanNumber, x, y, z), x, y, z, pos,
								ROI[pos]));
					}
				}
			}
	...
	public double[] getSingleBoldResponse(int scanNumber, int x, int y, int z) {
		assert (scanNumber > 0
				&& scanNumber <= this.currentScans) : "getSingleBoldResponse: scanNumber is out of range!";
